...
const SectorDelayTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white border border-slate-200 p-3 rounded-lg text-xs space-y-1 shadow-xl text-slate-900">
        <div className="font-bold text-slate-900">Sector: {data.sector}</div>
        <div className="text-slate-600">
          Projects: <span className="font-bold text-slate-900">{data.project_count}</span>
        </div>
        <div className="text-slate-600">
          Average Delay Risk: <span className="font-bold text-amber-600">{data.avg_delay_risk}%</span>
        </div>
        <div className="text-slate-600">
          Highest Project Risk: <span className="font-bold text-red-600">{data.highest_project_risk}/100</span>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded bg-teal-50 text-teal-700 border border-teal-200 flex items-center justify-center flex-shrink-0">
        ...
        {priority_distribution.P1 > 0 && (
          <button
            ...
            className="px-3.5 py-1.5 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 rounded text-xs font-bold transition flex items-center gap-1.5 shadow-sm"
          >
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900">Delay Risk by Sector</h3>
              <span className="text-[10px] px-2 py-0.5 rounded bg-slate-100 text-slate-600 font-mono border border-slate-200 font-semibold">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {sortedSectorData.length > 10 && (
                <button
                  ...
                  className="text-[11px] text-teal-700 hover:underline font-bold flex-shrink-0 ml-2"
                >
          ...
          <div>
            <div className="flex items-center justify-between border-b border-slate-200 pb-2">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded bg-red-50 text-red-700 font-bold border border-red-200">
                  P1 Priority Action
                </span>
              </h3>
              <button
            ...
            <p className="text-xs text-slate-500 mt-1 mb-3 font-medium">
                ...
                <tbody className="divide-y divide-slate-100">
                  {priorityQueue.map((p, idx) => (
                    ...
                    >
                      ...
                      <td className="py-2 px-2 font-bold text-slate-900 truncate max-w-[140px]" title={p.project_name}>
                        {p.project_name}
                      </td>
                      <td className="py-2 px-2 text-slate-600 text-[11px]">{p.sector}</td>
                      <td className="py-2 px-2 text-center font-bold text-red-600">{p.overall_risk_score}</td>
                      <td className="py-2 px-2 text-center font-mono text-amber-600 font-semibold">{p.time_overrun_prob}%</td>
                      <td className="py-2 px-2 text-center font-mono text-orange-600 font-semibold">{p.cost_overrun_prob}%</td>
                      <td className="py-2 px-2 text-center">
                        <span className="px-1.5 py-0.5 rounded bg-red-50 text-red-700 font-bold text-[10px] border border-red-200">
                          {p.priority_level}
                        </span>
                      </td>
                      <td className="py-2 px-2 text-slate-600 text-[11px] truncate max-w-[160px]" title={p.primary_delay_reason}>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-col space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-3">
          ...
          <div className="flex items-center gap-4 text-[11px] font-semibold text-slate-700">
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-red-600 inline-block"/> Critical (75-100)</span>
            ...
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-amber-500 inline-block"/> Moderate (25-49)</span>
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-emerald-600 inline-block"/> Low (&lt;25)</span>
          </div>
        </div>

        <div className="h-80 w-full rounded-lg overflow-hidden border border-slate-200 relative z-0">
          <MapContainer
          ...
          >
            ...
            {projects.slice(0, 400).map((proj) => {
              ...
              return (
                ...
                >
                  <Popup>
                    <div className="p-2 text-xs bg-white text-slate-900 rounded">
                      <div className="font-bold text-sm text-teal-800">{proj.project_name}</div>
                      <div className="text-slate-600 mt-1">{proj.ministry} | {proj.sector}</div>
                      <div className="mt-1 flex items-center justify-between">
                        <span>Revised Cost: ₹{proj.revised_cost} Cr</span>
                        <span className="font-bold text-red-600 ml-2">Risk: {proj.overall_risk_score}/100</span>
                      </div>
                      <button
                        onClick={() => onSelectProject(proj.project_id)}
                        className="mt-2 w-full py-1 bg-teal-700 hover:bg-teal-800 text-white rounded text-[10px] font-bold"
                      >
                        Open Full Review
                      </button>
                    </div>
